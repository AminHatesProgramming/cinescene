...
- Scene boundary detection, keyframe extraction, subtitle ingestion, mood/keyword tagging
...
It runs fully in the browser using `docs/data/catalog.sample.json`, supports install/offline caching, and demonstrates the offline crawler result with a real keyframe.
...
```powershell
.\.venv\Scripts\python.exe .\scripts\crawl_offline_videos.py "D:\Series" --title-prefix "My Library" --min-scene-sec 6 --threshold 0.4 --sample-fps 1
```
